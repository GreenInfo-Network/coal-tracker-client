...
(function (_PruneCluster) {
    ...
    var PruneCluster = (function () {
        ...
        // TFA added
        PruneCluster.prototype.FilterMarkers = function (markers, filter) {
            for (var i = 0, l = markers.length; i < l; ++i) {
...
var PruneClusterForLeaflet = (L.Layer ? L.Layer : L.Class).extend({
    ...
    // TFA
    FilterMarkers: function (markers, filter) {
        this.Cluster.FilterMarkers(markers, filter);
    },
    ...
    PrepareLeafletMarker: function (marker, data, category) {
        ...
        // per GDA, but not working :(
        if (data.title) {
            // console.log(marker);
            marker._icon.title = data.title;
        }
